...
ADD CONSTRAINT chk_loan_duration CHECK (
    DATERETOUR IS NULL OR ((DATEEMPRUNT + INTERVAL '14 DAYS' >= DATERETOUR) AND DATERETOUR != DATEEMPRUNT)
);


CREATE OR REPLACE FUNCTION check_commande_book_limit()
RETURNS TRIGGER AS $$
BEGIN
    IF (SELECT COUNT(*) FROM LIVRESCOMMANDE WHERE NOCOMMANDE = NEW.NOCOMMANDE) >= 3 THEN
        RAISE EXCEPTION 'Pas possible de commander plus que 3 livres!';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_check_book_limit
BEFORE INSERT ON LIVRESCOMMANDE
FOR EACH ROW EXECUTE FUNCTION check_commande_book_limit();
	...
	(100, 90100, '2023-12-01', '2023-12-10', 'Remis'),
	(101, 91001, '2024-04-01', NULL, DEFAULT),
	(1001, 90011, '2023-03-10', '2023-03-20', 'Remis'),
